...
princp=[]
try:
	prox= requests.get('https://github.com/Shamim1BD/SA/blob/main/Afroveal17.txt').text
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Shamim1BD/SA/blob/main/Afroveal17.txt').text
		ua=open('.bbnew.txt','w')
...
\033[1;32m==================================================
          Author     : SHAMIM SARKAR
          Telegram   : SHAMIM SARKAR       
          Facebook   : SHAMIM SARKAR
          Github     : SHAMIM1-BD
          Version    : \033[1;96mFREE//0.4\x1b[0m\033[1;31m [Updated] \x1b[0m
\033[1;31m==================================================           
\x1b[0;97m\x1b[1;43m         🍁  Mohammad SHAMIM  SARKAR 🥰          \x1b[0m
""")
#--------------------[ BAGIAN-SHAMIM ]--------------#
def login():
	try:
	...
	try:
		...
		for x in raq('input',{'value':True}):
			try:
				...
				pass
		rel = 'https://mbasic.facebook.com' + raq['action']; pos = sop(ses.post(rel,data=dat,cookies=cos).content,'html.parser'); req = ses.get(url,cookies=cos).json(); tok = req['access_token']; kot = open('.token.txt','w').write(tok); koc = open('.cok.txt','w').write(cok); SHAMIM = input('\n[+] tekan enter'); os.system('clear'); login()
	except Exception as e:
		print(e)
#------------------[ BAGIAN-MENU ]----------------#
def main():
